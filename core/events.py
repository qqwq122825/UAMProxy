...
class LogBus(QObject):
    ...
    # 重放阶段更新 → 连接表“模式”列（首次重放 / 续连重放）
    conn_replay_phase = Signal(str, str)       # (client_ip, phase_text)
    ...
    # 01 达阈值后自动阻断3366 → 同步界面按钮；01 通道继续
    threshold_3366_block = Signal(str, int, int)  # (client_ip, n01, threshold)
    # 录制目标达成后自动阻断同IP录制口3366（支持数量/消息覆盖率策略）。
    recording_goal_3366_block = Signal(str, str, str)  # (client_ip, reason, progress)
