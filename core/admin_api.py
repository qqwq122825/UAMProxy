...
class _AdminHttpServer(ThreadingHTTPServer):
    """HTTP server tuned for repeated proxy stop/start cycles."""

    # Without explicit address reuse Windows can leave the management port in
    # a state where the next proxy start fails to bind it.
...
class AdminApiServer:
    ...
    def start(self):
        if self._httpd is not None:
            return
        # Resolve/generate the initial token, but read it through server_ref
        # in each request so a password change in the desktop panel takes
        # effect without restarting the HTTP server.
        self.ensure_token()
        server_ref = self
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def _authed(self) -> bool:
                ...
                current_token = server_ref.token or ""
                return bool(current_token) and (
                    q_token == current_token
                    or h_token == current_token
                    or c_token == current_token
                )
            ...
            def do_GET(self):
                ...
                if p == "/api/type9/rules":
                    if not self._authed():
                        return self._need_auth()
                    from core.type9_special_rules import type9_hot_rule_store
                    return _json(self, 200, type9_hot_rule_store.snapshot())

                if p == "/api/plugin/keys":
                    from core.plugin_keys import plugin_key_store
                    result = {}
                    for ip, info in plugin_key_store.all_keys().items():
                        result[ip] = {
                            "key_hex": info["key"].hex().upper(),
                            "game": info.get("game", ""),
                            "uid": info.get("uid", ""),
                        }
                    return _json(self, 200, {"ok": True, "keys": result})

                if p == "/api/plugin/key":
                    src_ip = self.client_address[0] if self.client_address else "?"
                    _plugin_token = (app_config.get("plugin_api_token") or "").strip()
                    qs = parse_qs(urlparse(self.path).query)
                    if _plugin_token:
                        _req_token = (qs.get("token") or [""])[0].strip()
                        if _req_token != _plugin_token:
                            _event("WARN", "PluginAPI", f"[{src_ip}] Key请求token无效")
                            return _json(self, 401, {"ok": False, "error": "invalid plugin token"})
                    key_hex = (qs.get("key") or qs.get("key_hex") or [""])[0].strip()
                    game = (qs.get("game") or [""])[0].strip()
                    uid = (qs.get("uid") or [""])[0].strip()
                    if not key_hex:
                        _event("WARN", "PluginAPI", f"[{src_ip}] Key请求缺少key参数")
                        return _json(self, 400, {"ok": False, "error": "missing key"})
                    from core.plugin_keys import plugin_key_store
                    ok = plugin_key_store.set_key(src_ip, key_hex, game=game, uid=uid)
                    if not ok:
                        _event("WARN", "PluginAPI",
                               f"[{src_ip}] Key无效 len={len(key_hex)} (需要32 hex)")
                        return _json(self, 400, {
                            "ok": False,
                            "error": "invalid key (need 32 hex chars = 16 bytes)",
                        })
                    _event("INFO", "PluginAPI",
                           f"[{src_ip}] 收到Key game={game or '?'} "
                           f"key={key_hex}")
                ...
                if p == "/":
                    ...
                    page = _admin_page(server_ref.token or "")
            ...
            def do_POST(self):
                ...
                if p == "/api/login":
                    ...
                    _set_auth_cookie(self, current_token)
                    self.end_headers()
                ...
                if p == "/api/type9/rules/reload":
                    from core.type9_special_rules import type9_hot_rule_store
                    result = type9_hot_rule_store.reload(force=True)
                    ...
                    _event(
                        ...
                        f"active={result.get('active_rule_count')} "
                        f"error={result.get('last_error') or '-'}",
                    )
                ...
                if p == "/api/type9/rules":
                    try:
                        ...
                        ln = 0
                    if ln <= 0 or ln > 512 * 1024:
                        return _json(self, 400, {"ok": False, "error": "invalid_content_length"})
                    ...
                    try:
                        ...
                        from core.type9_special_rules import type9_hot_rule_store
                        result = type9_hot_rule_store.replace_document(document)
                    except Exception as exc:
                        _event("WARN", "Type9Rules", f"远程规则校验失败: {exc}")
                        return _json(self, 400, {
                            "ok": False,
                    ...
                    _event(
                        ...
                        "Type9Rules",
                        "远程规则已热更新 "
                        f"revision={result.get('document', {}).get('revision') or '-'} "
                        f"generation={result.get('generation')} "
                        f"active={result.get('active_rule_count')}",
                    )
        ...
        self._httpd = _AdminHttpServer((self.bind, self.port), Handler)
        # Port 0 is useful for tests; expose the actual bound port consistently.
        self.port = int(self._httpd.server_address[1])
        self._thread = threading.Thread(
            target=self._httpd.serve_forever,
            name="DFMProxy-AdminAPI",
            daemon=True,
        )
        self._thread.start()
