...
def process_dl_intercept(
    ...
    *,
    ...
    # 同时写 dl_intercept_log；emit_event_diag=False 时（如三角洲）不进「下发拦截详情」以保留简洁
    def _diag(msg: str):
...
def _intercept_one_3366_frame(
    ...
    plugin_game_id: str = "",
    pb_dl_cmd_blacklist: "list[str] | None" = None,
) -> "tuple[bytes, bool, bool, bool]":
    """
    对单个完整的 3366 帧做解密 → 拦截替换/块填充/PB命令黑名单 → 重加密。
    仅处理 MSG_DATA (40 13) 帧；其他帧原样返回。

    返回 (result_frame, was_str_replaced, was_chunk_dropped, was_pb_dl_hit)：
      was_str_replaced=True  → 字符串替换成功，调用方可据此停止后续字符串拦截
      was_chunk_dropped=True → 块下载帧已填充清零，调用方不应设置 DONE_KEY（持续拦截）
      was_pb_dl_hit=True     → PB 命令名黑名单命中并清零，调用方不应设置 DONE_KEY（持续拦截）
      三者均 False            → 未处理（非数据帧 / 解密失败 / 未命中）
    ...
    from core.protocol_3366 import (
        try_decrypt_4013_frame, try_decrypt_4013_frame_raw,
        _taes_encrypt, _aes128_cbc_encrypt,
        MSG_DATA, parse_3366_header, _extract_4013_cipher_by_spec,
    )

    # 诊断：三角洲仅写文件日志，避免弹窗刷屏；暗区仍进「下发拦截详情」
    def _diag(msg: str):
        log_bus.dl_intercept_log.emit(msg)
        if conn_label and not plugin_game_id:
            log_bus.dl_intercept_event.emit(conn_label, "diag", msg)
    ...
    # 插件 Key 游戏（Protobuf 明文）用宽松解密；暗区用严格解密
    if plugin_game_id:
    ...
    # ── PB 下行命令名黑名单（插件游戏专用，优先于块填充和字符串替换）─────────────
    # 检测到黑名单命令 → 等长混淆命令名（首字节 C→X）后重加密下发。
    # 等长替换保持 Protobuf 长度前缀不变，结构合法；客户端找不到对应 handler 直接忽略。
    # 不丢弃帧，避免 3366 序列号断档触发客户端重连。
    _seq_dl = info.get("seq", "?")
    if plugin_game_id and pb_dl_cmd_blacklist:
        for _dl_cmd in pb_dl_cmd_blacklist:
            _dl_bytes = (
                _dl_cmd.encode("ascii") if isinstance(_dl_cmd, str) else _dl_cmd
            )
            if _dl_bytes in plain:
                # 等长混淆：首字节 XOR 0x17（'C'=0x43 → 'T'=0x54），其余不变
                _fake_bytes = bytes([_dl_bytes[0] ^ 0x17]) + _dl_bytes[1:]
                _new_plain = plain.replace(_dl_bytes, _fake_bytes, 1)
                # 重加密
                _new_cipher = _taes_encrypt(_new_plain, key, iv)
                if not _new_cipher:
                    _pad = 16 - (len(_new_plain) % 16) or 16
                    _new_cipher = _aes128_cbc_encrypt(
                        _new_plain + bytes([_pad] * _pad), key, iv
                    )
                if _new_cipher:
                    _spec = _extract_4013_cipher_by_spec(frame)
                    _orig_enc = _spec[1] if _spec else (len(frame) - 25)
                    _new_hdr = bytearray(frame[:25])
                    _new_hdr[19:21] = struct.pack(">H", len(_new_cipher))
                    _tail = frame[25 + _orig_enc:] if _spec else b""
                    _new_frame = bytes(_new_hdr) + _new_cipher + _tail
                    _fake_str = _fake_bytes.decode("ascii", errors="replace")
                    msg_out = (
                        f"[{conn_id}] [PB_DL_BL] ✅ 命令名混淆 {_dl_cmd!r}→{_fake_str!r} "
                        f"帧{len(frame)}B→{len(_new_frame)}B"
                    )
                    log_bus.dl_intercept_log.emit(msg_out)
                    if conn_label:
                        log_bus.dl_intercept_event.emit(conn_label, "pb_dl_bl", msg_out)
                    return _new_frame, False, False, True
                else:
                    _diag(f"[{conn_id}] [PB_DL_BL] ❌ 命令名混淆后重加密失败 {_dl_cmd}")
                break
        else:
            # for 正常结束：未命中任一黑名单命令
            msg_scan = (
                f"[{conn_id}] [PB_DL_BL] ⚠ seq={_seq_dl} 帧{len(frame)}B "
                f"扫描 未命中下行命令黑名单"
            )
            log_bus.dl_intercept_log.emit(msg_scan)
            if conn_label:
                log_bus.dl_intercept_event.emit(conn_label, "pb_dl_scan", msg_scan)
            if not (search_str or "").strip():
                return frame, False, False, False

    # ── 块填充检测（暗区专用；插件游戏/三角洲跳过）────────────────────────────
    was_chunk_dropped = False
    if not plugin_game_id and app_config.get("ace_chunk_block_enabled") and not skip_chunk_block:
    ...
    if was_chunk_dropped:
        ...
        new_plain, was_replaced = process_dl_intercept(
            ...
            emit_event_diag=not bool(plugin_game_id),
        )
        if not was_replaced:
            # 原因[D]
            _diag(
    ...
    return new_frame, was_replaced, was_chunk_dropped, False
...
def process_dl_intercept_3366(
    ...
    # ── 已替换过：直接透传，跳过所有解密逻辑 ────────────────────────────
    # 例外1：块填充已启用 → 仍需逐帧解密检测块前缀
    # 例外2：pb_dl_cmd_blacklist 非空 → 需持续检测下行弹窗命令（如踢出通知），不可跳过
    _has_pb_dl_bl = bool(plugin_game_id and pb_dl_cmd_blacklist)
    if buf_dict.get(_DONE_KEY) and not app_config.get("ace_chunk_block_enabled") and not _has_pb_dl_bl:
        out = bytes(raw_buf)
        del raw_buf[:]
        return out
    ...
    # _GAME_KEY 存当前 game_id（"az"、"hok" 或 "0a92" 等）；任一方变化即重发。
    if conn_label:
        _new_gid = intercept_game_id or plugin_game_id or "az"
    ...
    while pos < len(raw_buf):
        ...
        if msg == MSG_DATA:
            ...
            # ── 小帧过滤：≤1000B 的帧既不含目标串也不是块下发帧，直接透传不解密 ──
            # 例外：pb_dl_cmd_blacklist 非空时（如踢出弹窗 CSGatewayKickPlayerNtf = 89B），
            # 小帧也需要解密检测，不可跳过。
            _STRING_MIN_FRAME = 1000
            _has_pb_dl_bl = bool(plugin_game_id and pb_dl_cmd_blacklist)
            if frame_total <= _STRING_MIN_FRAME and not _has_pb_dl_bl:
                output.extend(frame)
                pos = frame_end
                continue
            ...
            # 两项均不需要时（块填充未启用且无搜索串）：直接透传
            # 例外：三角洲下行配置了 PB 命令黑名单时仍需解密扫描，不可跳过
            if _skip_chunk and not _eff_search and not _has_pb_dl_bl:
                output.extend(frame)
                pos = frame_end
                continue

            modified, was_str_replaced, was_chunk_dropped, was_pb_dl_hit = \
                _intercept_one_3366_frame(
                    frame, kv, _eff_search, replace_str, conn_id, client_ip, conn_label,
                    skip_chunk_block=_skip_chunk,
                    plugin_game_id=plugin_game_id,
                    pb_dl_cmd_blacklist=pb_dl_cmd_blacklist,
                )
            ...
            if was_str_replaced:
                ...
                pos = len(raw_buf)
                break
            # was_chunk_dropped=True / was_pb_dl_hit=True：均不设 DONE_KEY，继续扫描后续帧
