...
class UserManager:
    ...
    @staticmethod
    def _norm_record_role(value) -> str:
        # v1.130.2 起所有录制统一进入玩家设备池；兼容读取旧字段。
        return "player"

    def add(self, username: str, password: str, expire: str = "never",
            note: str = "", allow_multi: bool = False, perm: str = "both",
            record_role: str = "player") -> bool:
        """返回 False 表示用户名已存在"""
        ...
        self._users.append({"username": username, "password": password,
                             ...
                             "record_role": self._norm_record_role(record_role)})
        self.save()
        return True
    ...
    def get_allow_multi(self, username: str) -> bool:
        """是否允许同账户多个游戏ID同时在线（默认 False）。
        关闭时同一游戏ID的录制+重放（即使 IP 不同）仍视为同一个人。"""
        for u in self._users:
            if u["username"] == username:
    ...
    def get_record_role(self, username: str) -> str:
        """录制端口身份统一为玩家设备池。"""
        for user in self._users:
            if user.get("username") == username:
                return self._norm_record_role(user.get("record_role"))
        return "player"

    def set_record_role(self, username: str, value: str):
        for user in self._users:
            if user.get("username") == username:
                user["record_role"] = self._norm_record_role(value)
                break
        self.save()
