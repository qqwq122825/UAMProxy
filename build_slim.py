...
_PINNED_EXPERIMENT_RE = re.compile(
    r'^APP_VERSION\s*=\s*["\'](v\d+\.\d+\.\d+(?:-[A-Za-z0-9._-]+)?)["\']',
    re.MULTILINE,
)
...
def bump_version() -> str:
    ...
    with open(VIEWS_FILE, "r", encoding="utf-8") as f:
        src = f.read()
    pinned = _PINNED_EXPERIMENT_RE.search(src)
    if pinned:
        version = pinned.group(1)
        print(f"[VERSION] 实验版本固定为 {version}，本次打包不自增")
        return version
