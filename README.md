...
- 录制按“代理账号 → 单次录制编号 → TCP连接”分层管理；代理账号是数据归属
  边界，游戏账号用于重放匹配，IP 只作连接关联和展示。
- 同公网 IP 下的不同代理账号互不共享录制池；同代理账号的并发 01/3366
  连接归入同一次 `recording_id`。
...
格式升级为 v6，保存 `recording_id`、`proxy_username`、来源 IP 和连接信息；
旧版 v2-v5 数据导入后标记为 `legacy-shared`，继续兼容已有录制文件。
